...
export function SidebarStatusCard({
  ...
  return (
    <section className="mt-8 rounded-[28px] border border-line/80 bg-white p-5 shadow-[0_8px_24px_rgba(14,30,56,0.05)]">
      ...
      <div className="mt-5 rounded-2xl border border-line/80 bg-panel px-4 py-3">
        <div className="flex items-start gap-3">
          <FolderOpen className="mt-0.5 h-4 w-4 text-navy-500" />
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-navy-500">
              Source Folder
            </p>
            <p className="mt-1 break-all text-xs leading-5 text-navy-700">
      ...
      {isDev ? (
        <button
          ...
          className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-line/80 bg-navy-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-navy-900 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {isReindexing ? (
      ...
      {status?.last_indexed_at ? (
        <p className="mt-4 text-xs leading-5 text-navy-600">
...
function StatusRow({ label, value }: StatusRowProps) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-2xl border border-line/80 bg-panel px-4 py-3 text-sm">
      <span className="text-navy-700">{label}</span>
      <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-navy-900 ring-1 ring-inset ring-line/80">
        {value}
      </span>
    </div>
  );
